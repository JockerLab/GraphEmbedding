            ...
            for optim_func in optimizers:
                for lr in learning_rates:
                    for reduction in reductions:
                        ...
                        encoder = EncoderRNNEdges(NODE_EMBEDDING_DIMENSION - ATTRIBUTES_POS_COUNT - 1, hidden_size,
                                                  num_layers, dropout, MAX_N).to(device)
                        decoder = DecoderRNNEdges(NODE_EMBEDDING_DIMENSION - ATTRIBUTES_POS_COUNT - 1, hidden_size,
                                                  num_layers, dropout, MAX_N).to(device)
                        model = Seq2SeqEdges(encoder, decoder).to(device)
                        ...
                        for epoch in range(N_EPOCHS):
                            ...
                            for row in test_input[0]:
                                ...
                                if data_len <= 0:
                                    continue
                                test_embedding = model.encode(row, data_len)
