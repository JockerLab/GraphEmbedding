...
class VAE3(nn.Module):
    def __init__(self, shapes, init_mean, init_std):
        ...
        self.decoder = nn.Sequential(nn.Linear(shapes[2], shapes[1]), nn.Tanh(), nn.Linear(shapes[1], shapes[0]))
    ...
    def forward(self, inputs, training=True):
        ...
        return BCE + KLD, BCE, KLD, x.view(-1)

    def loss_function(self, input, output, mu, gamma, batch_size=1):
        BCE = F.mse_loss(output, input, reduction='sum')
